...
//gets low, high, and mean of values in list and writes to output file
void ListApp::computeStats() {
	if (mList.isEmpty()) {
		mOutputStream << "List is empty; no stats computed.";
	}
	int low = 0, high = 0;
	//number of entries, sum of all entries
	int n = 0, sum = 0;
	double mean = 0.0;
	
	ListNode * pCurr = mList.getHeadPtr();
	
	//set low and high to first entry in case there are no others
	low = pCurr->getData();
	high = low;
	sum = high;
	n++;
	...
	while (pCurr != nullptr) {
		if (pCurr->getData() > high) {
			high = pCurr->getData();
		}
		if (pCurr->getData() < low) {
			low = pCurr->getData();
		}
		n++;
		sum += pCurr->getData();
	...
	mOutputStream << "Processed " << n << " entries." << endl
		<< "Low is " << low << endl
		<< "High is " << high << endl
		<< "Mean is " << mean << endl;
